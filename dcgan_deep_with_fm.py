...
def generator_model():
    ...
    # output
    # (N, 32, 32, 3)
    model.add(Conv2DTranspose(filters=3, kernel_size=5, padding='same', strides=2))
    model.add(Activation('tanh'))
...
def discriminator_model():
    ...
    # (N, 16, 16, 128)
    model.add(Conv2D(filters=128, kernel_size=5, padding='same', strides=2, input_shape=(32, 32, 3)))
    model.add(BatchNormalization(epsilon=1e-5, momentum=0.9))
    model.add(LeakyReLU(0.2))
