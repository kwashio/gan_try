...
def generator_model():
    ...
    # (N, 100)
    model = Sequential()
    model.add(Dense(4 * 4 * 512, input_dim=100, kernel_initializer='TruncatedNormal'))
    ...
    # (N, 8, 8, 256)
    model.add(Conv2DTranspose(filters=256, kernel_size=5, strides=2, padding='same', kernel_initializer='TruncatedNormal'))
    model.add(BatchNormalization(epsilon=1e-5, momentum=0.9))
    model.add(Activation('relu'))

    # (N, 16, 16, 128)
    model.add(Conv2DTranspose(filters=128, kernel_size=5, strides=2, padding='same', kernel_initializer='TruncatedNormal'))
    model.add(BatchNormalization(epsilon=1e-5, momentum=0.9))
    model.add(Activation('relu'))

    # output
    # (N, 32, 32, 3)
    model.add(Conv2DTranspose(filters=3, kernel_size=5, padding='same', strides=2, kernel_initializer='TruncatedNormal'))
...
def discriminator_model():
    ...
    # (N, 16, 16, 128)
    model.add(Conv2D(filters=128, kernel_size=5, padding='same', strides=2, input_shape=(32, 32, 3), kernel_initializer='TruncatedNormal'))
    #model.add(BatchNormalization(epsilon=1e-5, momentum=0.9))
    model.add(LeakyReLU(0.2))

    # (N, 8, 8, 256)
    model.add(Conv2D(filters=256, kernel_size=5, padding='same', strides=2, kernel_initializer='TruncatedNormal'))
    #model.add(BatchNormalization(epsilon=1e-5, momentum=0.9))
    model.add(LeakyReLU(0.2))

    # (N, 4, 4, 512)
    model.add(Conv2D(filters=64, kernel_size=5, padding='same', strides=2, kernel_initializer='TruncatedNormal'))
    #model.add(BatchNormalization(epsilon=1e-5, momentum=0.9))
    model.add(LeakyReLU(0.2))
    ...
    # model = Sequential()
    # model.add(Conv2D(filters=64, kernel_size=5, padding='same', strides=2,
    #                  input_shape=(32,32,3),
    ...
    # model.add(Flatten())
    # model.add(Dense(256))
    # model.add(LeakyReLU(0.2, name='last_hidden_layer'))
    # model.add(Dense(1))
    # model.add(Activation('sigmoid'))
...
def train():
    ...
    for epoch in range(NUM_EPOCH):
        ...
        for index in range(num_batches):
            ...
            print(
                "epoch: %d, batch: %d, g_loss: %f, d_loss: %f, fm_loss: %f" % (
                epoch, index, g_loss[0], d_loss, g_loss[1]))
